...
Pop_Data_ineq 		PopBestIneq;

//Select the most violated inequalities
Class_PopClique		PopClique;
Class_PopClique		PopGenClique;
Class_PopTriangle 	PopTriangle;
Pop_Ineq_Wheel 		PopWheell;
Pop_Ineq_Wheel 		PopBiWheell;
Pop_Ineq_SDP		PopVecProp;
...
int main (int argc,char *argv[])
{
    ...
    sdpEdgeConst.size = 0;
    sdpEdgeConst.varI.clear();sdpEdgeConst.varJ.clear();
...
}else{ //test of heuristic methods
...
//      Set_IneqFuncObj(instance, 6669); //This way we limit our next solutions to be inferior than first solution. Impotant when we use Upperbound from Nikiforov
   ...
   if ((SDP_SEP == -1)  || (SDP_SEP == -3)) TYPE_SOLVER_BB = TYPE_SDP_BB;
   else 	      			    TYPE_SOLVER_BB = TYPE_SIMPLEX_BB; //  TYPE_IPM_BB
...
//      Print_ListBB_allActive (ListBB); cin.get();
  ...
  // 	      bestLowerBound = 18512;
  	...
  	if (R == false) 	  goto END_OF_BB; // Signal of end of BB
...
//  		PrintScreen_BB(counter,ListBB.size() ,bestLowerBound,bestUpperBound,100*(bestUpperBound-bestLowerBound)/bestLowerBound, getCurrentTime_Double(start_BB));
      ...
      if ((*it_branch).upperbound > bestLowerBound + 0.99) R = IsOK; //Resume
      else 	R = NotOK; //fathom by bound
      LocalUb = (*it_branch).upperbound ;

      if (BY_PARTITION_BB) Partitions = (*it_branch).Partitions;
      else                 fixvar = (*it_branch).fixvar;
	...
	//	cout << (*it_branch).Level_tree << endl; cin.get();
		...
		//Fathom by bound ??
      if (LocalUb >= bestLowerBound + 0.99 && R == IsOK){ //Fathom by Bound
...
inline void Set_BranchingRule(const T_Instance &instance, const bool &BY_PARTITION_BB)
{
   ...
   if (BRANCHING_RULE_BB == R7) {//PseudoCost Branching
   if (BY_PARTITION_BB){
...
bool R = true;
	  ...
	  if (TYPE_SOLVER_BB != TYPE_SDP_BB) 	CreatNewCandidates_FixPatitions_RedCost_BB(ListBB, instance, Partitions, bestLowerBound);
	  else 					CreatNewCandidates_FixPatitions_BB(ListBB, instance, Partitions, bestLowerBound);
      }
      else {
  ...
  const bool  &BY_PARTITION_BB,  const int &TYPE_SOLVER_BB,  const bool &NEW_TASK, double *bestLowerBound,const  bool  &sdp_edge_type)
{
	...
	if (BY_PARTITION_BB == true){ // it is the old way (have to change it)
...
//      R =  Set_TrivialVertices_inPartition_BB(instance,*Partitions,bestLowerBound); //If R not true ---> Prunne by Integer solution (not helping)
    ...
    if (TYPE_SOLVER_BB == TYPE_SDP_BB) R  = SolveMosek_SDP_for_BB (instance, fixvar); //solve using SDP solver
    else 				                       R  = SolveMosek_LP_for_Branch(instance, r_BB, task_BB, env_BB,   TYPE_SOLVER_BB ,  fixvar ,NEW_TASK ); //TYPE_SIMPLEX_BB
...
//	}
	...
	for (int i=0; i< size -1; ++i ){
		...
		if ( instance.cij.barc_i[n_var]  ==  v_I ){ //it means the exit a a h,i... thus we have to fix h,j
			n_Z = v_J;
			n_H = instance.cij.barc_j[n_var] ;
		} else if ( instance.cij.barc_i[n_var]  ==  v_J ){ //it means the exit a a h,i... thus we have to fix h,j
			n_Z = v_I;
			n_H = instance.cij.barc_j[n_var] ;
		} else if ( instance.cij.barc_j[n_var]  ==  v_I ){ //it means the exit a a h,i... thus we have to fix h,j
			n_Z = v_J;
			n_H = instance.cij.barc_i[n_var] ;
		} else if ( instance.cij.barc_j[n_var]  ==  v_J ){ //it means the exit a a h,i... thus we have to fix h,j
			n_Z = v_I;
			n_H = instance.cij.barc_i[n_var] ;
		}
...
void Fix_N_Vertices_and_CreatPartitions(T_Instance &instance, std::set <T_Branch> &ListBB_Orig, const int & qtNb)
{
  ...
  int counter, selVer;
  set<RankVertex> ranked;
  set<RankVertex>::iterator it;
  vector<int> ranked_v;
...
//    else 		 TYPE_SOLVER_BB = TYPE_IPM_BB;
   ...
   if (TYPE_SOLVER_BB == TYPE_SDP_BB) SolveMosek_SDP_for_BB (instance, fixvar);
   else 			      SolveMosek_LP_for_Branch(instance, r_BB, task_BB, env_BB, TYPE_IPM_BB,  fixvar, NEW_TASK);
  ...
  seqqq_BB = 0;
   ...
   while(ListBB.size() > 0){
	...
	if (TotalNbVertices_Partition((*it_branch).Partitions) >= qtNb){ //good to go
		//insert int the original list
		ListBB_Orig.insert( (*it_branch) );
	}else { // generate candidates from (*it_branch)
		...
		R = Select_validVertex_from_RankedVector_BB(Partitions ,ranked_v, &selVer);
		 ...
		 if (R == true){
		    if (Partitions.size() < K)  Partitions.resize(Partitions.size() + 1);
		    flag = true;
		    for (int part=0; part < Partitions.size(); part++){
			    if (Partitions[part].size()!= 0 || flag){//add in just one void partitions
...
//    Partitions = Part_Orig;
   ...
   if (R == true){
   	...
   	for (int i=0; i < Partitions.size(); i++){
		if (Partitions[i].size()!= 0 || flag) {//add in just one void partitions
...
void FixVarible_to_ONE_RCisBig_BB (const T_Instance &instance, std::vector< std::vector<int> > &Partitions, double *bestLowerBound,double*redcost)
{
   ...
   for (int part=0; part<Partitions.size(); ++part)
	...
	for (int i=0; i<instance.DIM; ++i) if (InPartition[i] == -1){
	  for (int part=0; part<Partitions.size(); ++part){
	  val = 0.0;
	    for (int j=0; j<Partitions[part].size(); ++j){//vertex in partition
	      I = i+1;
...
// 		cout << "instance.ObSol" << instance.ObSol; cin.get();
      if (instance.ObSol <= *bestLowerBound + 0.99)  return void(); //Fathom by bound
    }
...
//   PrintPartitionICH(Partitions_old);
  ...
  if (Partitions_old.size() < K ) return -1.0; //end of function
...
//     PrintPartitionICH(Partitions);
  ...
  for (unsigned i=0; i < Partitions.size(); i++ ){
    ...
    for ( unsigned ii =0; ii< Partitions[i].size(); ii++) if (!VerInPartition[Partitions[i][ii]]){//can not change the fixed vertices
...
//     printvector(VerInParti);
  ...
  for (long i=0; i<instance.edge_nb; i++){
    ...
    if (IsBinary(val)){ // val is binary
      if ((I == -1 && J != -1) || (I != -1 && J == -1)){ // one of the partitions should not be in a parttition


	//Check val is 0 or 1.0
	if ((val -0.9 )>= 0.0){
	  //v_out is the vertex that is not fixed, yet.
	  if ( I != -1) {P_in = I; v_out = instance.cij.barc_j[i]-1;}
	  else 	      {P_in = J; v_out = instance.cij.barc_i[i]-1;}
...
// 	    cout << "val= " << val_P<<endl;
	    if (j != P_in){
	      if (val_P > val_Pbst) val_Pbst = val_P;
	    }else { // Partition is that he is already
		val_Pin = val_P;
	    }
  ...
  // 	    cout << "val_Pin=" << val_Pin << ", val_Pbst=" << val_Pbst << ", compGap = "<< compGap << ", gapUL = " << gapUL; cin.get();
	  ...
	  if (compGap > gapUL){ //thus v_out should be in partition P_in
...
// 	newPartitions = Partitions;
  ...
  for (int i = 0 ; i < instance.DIM; i++ ){
    if (VerInParti[i] == -1){ // it is not in a partition
	...
	cout << "***** v=" << i << endl;
	  ...
	  cout << "gapUL=" << gapUL << ", compGap="<<compGap << ", val_bst="<< val_bst << ", val_SeconBest="<< val_SeconBest; cin.get();
	  // SHould put v_out in partition P_in
...
//       cout << "chegou val= " << val ;
      if (val < epslon_val){// val is close to 0
...
// 	    cout << "val= " << val_P<<endl;
	...
	if (compGap > gapUL - 1.0){ //thus v_out should be in partition P_in
...
double Val_newVertex_inPartition( const T_Instance &instance, const std::vector< std::vector<int> > &Partitions, const int &P_in, const int &v_out )
{
  ...
  for (int i=0; i<Partitions.size(); i++)if (i != P_in){
      for (int ii=0; ii<Partitions[i].size();ii++){
	I = Partitions[i][ii] +1;
	aux_edge = instance.indice_cij[I+J*(instance.DIM+1)];
...
//	PrintPartitionICH(Partitions );  cin.get();
   ...
   //Calcule all the not in partition with
    Val_OtherVert_byPartition = 0.0;
   for (int i=0; i<instance.DIM ; i++) if (NotinPartition[i]) {
   		LargestVal  = 0.0;
...
bool ImprovePartition_FixMoreVertices2(const T_Instance &instance, std::vector< std::vector<int> > &Partitions, const double  &bestLB)
{
  ...
  int counter = 0;
   ...
   if (counter >= instance.DIM-1 ||  counter < instance.DIM*0.3) return true; //avoid segmentation error UpperBoundSmallestEigenvalue_Partition
  ...
  //2nd part Calculate the Upper bound of maxKcut for edge not in partition
   ...
   for (int i=0; i<instance.DIM ; i++) if (NotinPartition[i]) {
     ...
     for (int j=0; j<instance.DIM; j++) if (NotinPartition[j]  && i != j){
	LargestVal  = 0.0;
	    for (int p=0; p<Partitions.size(); p++){
...
//   printvector(NewVal);
  ...
  for (int i=0; i<instance.DIM; i++){
	...
	if (i != j){
	  ...
	  if (aux_edge != -1 && ( !InPartition[i] ||  !InPartition[j])){//it is a valid edge (at least i or j cannot be in partition to be count )
...
// 	cout << "The correct = " << Get_valueOfPartition (instance,Partitions) << "The calculate = " << UpperBoundSmallestEigenvalue_Partition(instance,verfVertex); cin.get();
   ...
   for (int i=0; i<instance.DIM -1; i++){
	if (!verfVertex[i]){
		...
		if (LargestVal - secondLrg > LargestDif_ALL) LargestDif_ALL = LargestVal - secondLrg ;
	}//end if I
   }//end for I
  ...
  for (int i=0; i<instance.DIM; i++){
	if (!verfVertex[i]){//not yet fixed
	 	LargestVal = 0.0;
		secondLrg = 0.0;
		for (int j=0; j<Partitions.size(); j++){ // simulate that vertex i goes to partition j
			sumVal = Val_newVertex_inPartition(instance, Partitions, j , i );    // VALUE if vertex i is in partition j

			if (sumVal >= LargestVal){ //must be >=
				secondLrg = LargestVal ;
				LargestVal = sumVal;
				part_j = j;
			} else if (sumVal > secondLrg ) {
		...
		if( (LargestVal - secondLrg) > gap){
			cout << "gap=" << gap << ", dif=" << (LargestVal - secondLrg); cin.get();
			//insert in partition
...
//		cout << "foi daki"; cin.get();
  ...
  for (int i=0; i<instance.DIM; i++) if (!verfVertex[i]){
      val = 0.0;
      for (int j=0; j<instance.DIM; j++) if (verfVertex[j]){
...
//    selectedEdge = ChooseEdge_ConsiderPartition_BB(instance, Partitions); // select edge that has at least one vertex fixed
   ...
   if (selectedEdge == -1){ // all the edges are integer
	...
	*selectedVertex = instance.cij.barc_i[selectedEdge] - 1; // start by 1 in cost but in Partitions the vertices start by 0
	if (verfVertex[*selectedVertex]){
		*selectedVertex = instance.cij.barc_j[selectedEdge] - 1;
			if (verfVertex[*selectedVertex]){ // may enter here if we are using ImprovePartition_FixMoreVertices
				*selectedVertex = Find_firstVertexNotInPartition_BB (verfVertex,instance.DIM);
  ...
  if (*selectedVertex != -1) return true;
  else{
...
//   cout << "instance.DIM = " << instance.DIM; cin.get();
  for (unsigned i=0; i < instance.DIM-2; i++)
    for (unsigned j= i+1; j <instance.DIM-1; j++)
      for (unsigned h=j+1; h<instance.DIM; h++) if(!GetExceptionTriangle(instance, i,j, h) ){
//	  cout << i << j << h; cin.get();
	  ...
	  sumVal +=ValueXij(h+1,j+1,instance);
    	  ...
    	  if (sumVal < epslonICH) continue; // exit this iteration
    	  ...
    	  if (vecVertex[I] == -1 || vecVertex[J] == -1 || vecVertex[H] == -1){
			...
			newPartition.push_back(I); newPartition.push_back(J); newPartition.push_back(H);
  ...
  for (unsigned i=0; i < instance.DIM; i++){
	...
	*/
	...
	if (Partitions.size() < instance.DIM){ //it is a final feasible solution
...
//				cout << "val_e = " << val_e;
//				cout << " redcost[aux_edge]  = " << redcost[aux_edge] ;  cin.get();
				if (val_e <= epslon){//it means that value is zero
				...
				}else if (val_e >= 1.0 - epslon){ //val is 1.0
					if ((redcost_BB[aux_edge]) > gap){
						newFixVar.var = aux_edge;
						newFixVar.val = 1.0;
						(*fixvar).push_back(newFixVar);
...
void Creat_TwoCandidates_BB (std::set <T_Branch> &ListBB, T_Instance &instance, const std::vector<T_fixVar> &fixvarOrig, double &bestLB, const double &selectedEdge)
{
  ...
  int InitialsizeFix = new_vecfixvar.size();
   ...
   for (int i=0; i<2 ;i++){
    ...
    if (i == 0) new_vecfixvar[new_vecfixvar.size()-1].val = 0.0;
    else 	    new_vecfixvar[new_vecfixvar.size()-1].val = 1.0;
  ...
  for (long i=0; i<instance.edge_nb; i ++){
     if ( !IsBinary(instance.varS[i])){
	  ...
	  if (val< 0.0) // set just positive values
	    val*= -1.0;
	  if (val < bestVal){ // not take the ones that cost 0
	    bestVal = val;
	    bestEdge = i;
	  }
     }//end IF
...
//	   PrintPartitionICH(Partitions);
  for (int i=0; i<instance.DIM; ++i)if(AllowVertices_easy[i]){
    val_i = 0.0;
  	I = i+1;
  	for (int j=0; j<Partitions.size();  ++j)
		for (int jj=0; jj<Partitions[j].size(); ++jj){
...
//				cout << "auxV=" << auxV << ", val_i= " << val_i; cin.get();
   ...
   best_val = 10000.0;
  //find second vertex
  for (int j=0; j<instance.DIM; ++j) if (j != bestVertex_i) {
    val_j = 0.0;
  	I = j+1;
...
//     if ( !IsBinary(instance.varS[i]))
	...
	if  (( b_j || b_i) && !(b_j && b_i)){ // one of the vertices should be already fixed
		  val = instance.varS[i] - fixVal; //Set is ordered by small to large
		  if (val< 0.0) // set just positive values
		    val*= -1.0;
		  if (val < bestVal){ // not take the ones that cost 0
...
long FindBestEdge_score ( std::set<int>UsedEdges,const std::vector<T_PseudCost_BB > &PseudoCcost , const double &Mu)
{
	...
	double val;
	long vetx = -1;
	for (long i=0; i<PseudoCcost.size(); ++i) if (UsedEdges.count(i) == 0){
...
int FindBestVertex_score (const vector<bool> &validVertex,const std::vector<T_PseudCost_BB > &PseudoCcost , const double &Mu)
{
	...
	double val;
	int vetx = -1;
	for (int i=0; i<PseudoCcost.size(); ++i) if (validVertex[i]){
		val = PseudoCcost[i].Get_Score(Mu);
		if (val >= valMax){
			valMax = val;
			vetx = i;
...
//	PrintPartitionICH(Partitions); cin.get();
    for (int p=0; p<Partitions.size() ; ++p)
     for (int pp=0; pp<Partitions[p].size(); ++pp)
	  ...
	  for (int i=0; i<instance.DIM && RESUME; ++i)if(AllowVertices_easy[i]){
	  max_val = 0.0;
	  	for (int part =0; part< Partitions.size(); ++part){
			Partitions[part].push_back(i);
    ...
    val_i = 0.0;
  	...
  	for (int j=0; j<Partitions.size();  ++j)
		for (int jj=0; jj<Partitions[j].size(); ++jj){
			...
			if (auxV <0) auxV *=-1.0; //Just module
			 val_i += auxV;
			 }
		}
...
//   PrintPartitionICH(Partitions);
  ...
  for (long i=0; i<instance.edge_nb; i ++){
     if (!IsBinary(instance.varS[i])){ //add allowed vertices
        b_j = AllowVertices_easy[instance.cij.barc_j[i]-1];
	b_i = AllowVertices_easy[instance.cij.barc_i[i]-1];
	if ( b_j || b_i) // one of the vertices should be already fixed
	  if (-mult*instance.cij.barc_v[i] < best_val){ // analyze if is the best value
...
inline void Transform_SDPsol_2_LPsol_for_BB(T_Instance &instance, double*barx, const double &LBsdp, const double &UBsdp)
{
  ...
  for( int j=0; j<instance.DIM; j++)
    for(int i=j+1; i<instance.DIM; i++){
      ...
      J = j+1;
	  aux_edge = instance.indice_cij[I+J*(instance.DIM+1)];
	  if (aux_edge != -1){ //valid edge in graph
...
T_Instance instance = instance_orig;
 ...
 if ((SDP_SEP == -1)  || (SDP_SEP == -3)) typeSolver = TYPE_SDP_BB;
 else 	      			    	  typeSolver = TYPE_IPM_BB ; //  TYPE_SIMPLEX_BB
 ...
 if (BY_PARTITION_BB)
  ...
  if (CLEAN != 0) {
    if ((SDP_SEP == -2) || (SDP_SEP == -3)) CLEAN = 2;
    else		     		     CLEAN = 3;
  }
...
//		Clean_uselessInequalities (instance);
	...
	if (RESUME) {
	    if (i == 0) gapImp = 1.0;
	    else 	gapImp = (PrvObj- instance.ObSol)/PrvObj;
...
//   double 	MAXTIME_ITE_Begin = MAXTIME_ITE;
  ...
  if (SDP_SEP == 0) {
    ...
    NBMAXINEQ = 1000;
     ...
     DesactivateAll();
     if  (SDP_SEP == -1 && !SDP_EdGE_TYPE ) ActiveJustTriWhell();

     if  (SDP_SEP == -1 || SDP_SEP == -3) NBMAXINEQ = NBINEQSDP; //sdp
     else 				   NBMAXINEQ = instance.DIM*0.50; // 10% of DIMBARVAR (for LP_EIG)
  ...
  bool OPTMAL_DONE ,LP_TO_OPTM; OPTMAL_DONE = LP_TO_OPTM = false;
...
//      cout << "Starting IPM : " << endl;
	...
	LP_TO_OPTM = true;
	  ...
	  { // if LP method we will set the initial solution !!!
		  ...
		  if (eco > eco2) instance.ObSol -= eco; //it means that Pre treatment was more effective than trivial solution
		  else		  instance.ObSol -= eco2;
	  ...
	  else
	  {
	    DO_EIG_INEQ_FROM_SDP = false; // only first iteration of SDP_EIG
	      //Set solver
	      if (LP_TO_OPTM){
		if (SDP_SEP == -2) 		setSolver(instance, r,task, env, 1); // solve  IPM  to optmality
		else if (SDP_SEP == -3)		setSolver(instance, r,task, env, -1); // solve  IPM  to optmality (SDP)
		else		    	     	setSolver(instance, r,task, env, SDP_SEP);
		...
		if ((SDP_SEP == -2)  || (SDP_SEP == -3))	OPTMAL_DONE = false;
		else 		  				OPTMAL_DONE = true;
	      }
      }
	...
	//stopping criterias Iteration improvement
      if (OPTMAL_DONE){
		...
		if (i== 0) firstSolVal = instance.ObSol;
		PrvObj = instance.ObSol;
      }
...
// 	MAXTIME_ITE = 0.0; (there is no sense in putting this here)
	...
	countNot ++;
      ...
      //Selective separation or Printing final solution !!!
      if (RESUME) 	 Selective_Separation(instance);
//       else 		 WriteFinalResult_File (fileResult,instance,start,i);
  ...
  if (PRINT_ITERATIONS){
    ...
    if (SDP_SEP == -2)      setSolver(instance, r,task, env, 1); // solve IPM to optimality
    else if (SDP_SEP == -3) setSolver(instance, r,task, env, -1); // solve IPM to optimality
    else 		    setSolver(instance, r,task, env, SDP_SEP);
...
void PreTrea_Div2Conq_rank_exploreAll(T_Instance &instance)
{
  ...
  if (ICH != 0) prop = (double)ICH/200.0;
  else prop = 0.25;
  ...
  //end rank
   ...
   for (int i=0; i <(int)begin_v.size(); i++)
     for (int j=i+1; j <(int)begin_v.size(); j++){
       ...
       if (end_v[j]-begin_v[j] > 2){ //have to have at least 3 vertices in the group
	ClearInstance(instance2);
...
void SubGraphTreat_by_ViolatedIneq(T_Instance &instance)
{
  ...
  Class_PopTriangle 	PopTriangle_origem = PopTriangle;
  Class_PopClique	PopClique_origem = PopClique;
  Class_PopClique	PopGenClique_origem = PopGenClique;
  Pop_Ineq_Wheel        PopWheell_origem = PopWheell;
  Pop_Ineq_Wheel 	PopBiWheell_origem = PopBiWheell;

  // Population of inequalities
  Pop_Data_ineq 		PopBestIneq_origem = PopBestIneq;
...
//    size_max = K*10;
  ...
  Rank_by_incidentWeight(ranked,instance);
	...
	{
		cout << "Pegou differenca " << ranked.size(); cin.get();
	}
...
void PreTrea_Div2Conq(T_Instance &instance)
{
  ...
  if (ICH != 0) prop = (double)ICH/100.0;
  else prop = 0.5;
...
inline void Rank_by_incidentWeight(std::set<RankVertex> &ranked, const T_Instance &instance )
{
 double cont_i;
 int aux_edge, I, J;
 vector<int> ranked_v;
 ...
 for (int i=0; i<instance.DIM; i++){
   ...
   if (cont_i > 0){ //jUST POSITIVES
   RankVertex rkVer(i+1, cont_i);
   ranked.insert(rkVer);
   }
 }//end FOR i
...
void Rank_by_degree(std::set<RankVertex> &ranked, const T_Instance &instance )
{
 double cont_i;
 int aux_edge, I, J;
 ...
 //std::set<RankVertex>::iterator it;
 //for (it=ranked.begin(); it!=ranked.end(); ++it)
...
inline void DynamicIneqActivation(const T_Instance &instance, const double &gapImp)
{
	...
	if (gapImp < minActGapAll )		ActiveAllIneq() ;
	else  	if (gapImp < minActGapTRI )	ActiveJustTriWhell();
...
double leftside;
 ...
 //printSolution_screen(instance); cin.get();
  ...
  for (int i=0; i< (int) CONST_BB.size(); ++i){
    ...
    if (CONST_BB[i].Origem > 30 && CONST_BB[i].Origem < 50) continue;
    ...
    if (CONST_BB[i].bkc == MSK_BK_UP){// Bound key (<=)
...
void Clean_uselessInequalities (T_Instance & instance)
{
 ...
 //printSolution_screen(instance); cin.get();
  ...
  for (int i=0; i< (int) instance.CONST.size(); i+=random){
    ...
    if (instance.CONST[i].Origem >= 30 && instance.CONST[i].Origem < 50) continue; /* Node and edge formulations... cannot be removed*/
...
//     cout << "leftside = " << leftside << "instance.CONST[i].blc = " << instance.CONST[i].blc << endl ; //cin.get();
    ...
    if (instance.CONST[i].bkc == MSK_BK_UP){// Bound key (<=)
...
inline void Set_validIneq_in_MainInstance_justEig (T_Instance & destination, const T_Instance & source,  std::vector<int> &origen_indice)
{
  ...
  for (int i=0; i< (int) source.CONST.size(); i++ ){
    ...
    if (source.CONST[i].Origem == 9){
      if (source.CONST[i].bkc == MSK_BK_UP){// Bound key (<=)
...
SDP_SEP = 1;
  ...
  for (int i=0; i<nb_ITE && RESUME; i++){
	...
	Obj1 = instance.ObSol;
      ...
      double gapImp = (PrvObj- instance.ObSol)/PrvObj; if (i == 0) gapImp = 1.0;
      PrvObj = instance.ObSol;
...
//     cout << "Found = " << Found << endl;cin.get();
    ...
    }else if (PopBestIneq.size() > 0) return true;
	  else	   return false;
...
//
//
//   setParam_cplex(cplex,logfile);


  if (SIMPLEXITE == 0){
    setVars_cplex(vars, instance,env);
    SIMPLEXITE = 1;
    setObjFunction_cplex(vars, instance, model,env);
  }

  setConstraints_cplex(vars, instance, model,env);


  double initialtime = getCurrentTime_Double(start);
    /// Solve
  cplex.clear();
  cplex.extract(model);
  cplex.solve();
  ...
  if (cplex.getStatus() == IloAlgorithm::Optimal) {
	  ...
	  //PrintValsVarCplex(instance, vals);

	 // env.out() << "Values = " << vals << endl;
	  //To be done ( setting )

	 //cout << "Time = " << getCurrentTime_Double(start); - initialtime ;

	  instance.ObSol = cplex.getObjValue();

	  for(int i=0; i<instance.edge_nb; i++)
	      instance.varS[i] = vals[i];

	  time_IPM_iteration = getCurrentTime_Double(start); - initialtime ;
	  //printSolution_screen(instance);
  }
...
void setConstraints_cplex(IloNumVarArray &vars, T_Instance &instance, IloModel &model, IloEnv &env)
{
  ...
  //cout << "LastConstraint = " << LastConstraint << ", instance.CONST.size() = " << instance.CONST.size(); cin.get();

  for(int i=LastConstraint; i< (int) instance.CONST.size(); i++){
     IloNumExpr  xpr(env);

     for (int j=0; j<instance.CONST[i].vars.size(); j++)
	xpr += instance.CONST[i].aval[j]*vars[instance.CONST[i].vars[j]] ;

     IloRange cst_a(env,instance.CONST[i].blc,xpr,instance.CONST[i].buc,"");
  ...
  LastConstraint = instance.CONST.size() -1;

}
inline void setObjFunction_cplex(IloNumVarArray &vars, T_Instance &instance, IloModel &model, IloEnv &env)
{
    /******* Objective function *****/
    /* max sum(val(edge)*(1-var(edge))) */
    IloObjective obj = IloMaximize(env);
    {
      IloExpr xpr(env);//create expression to be added in the obj and after in the model
      for (int i=0; i<instance.totalVars; i++){
	xpr += instance.cij.barc_v[i]*(1-vars[i]);
      }
      obj.setExpr (xpr);
    }

	//cout << "obj ="  << obj; cin.get();
    model.add(obj);
}
...
inline void setParam_cplex(IloCplex &cplex, stringstream &logfile)
{
  ...
  cplex.setOut(cout);// logfile or cout
}
...
inline void Set_CombinatorialConstraints_MOSEK_SDP(const T_Instance &instance,MSKrescodee &r, MSKtask_t &task, const double &LBsdp, const double &UBsdp)
{
  ...
  for (int i=0; i < (int) instance.CONST.size()  && r==MSK_RES_OK; i++){
      ...
      for (int j=0; j<nb; j++ ){ // setting value in allocated vectors
   ...
   for( int j=0; j<instance.DIM && sdpEdgeConst.newadd < qtIneq; j++)
    for(int i=j+1; i<instance.DIM && sdpEdgeConst.newadd < qtIneq ; i++){
	  ...
	  if (aux_edge != -1){
	      ...
	      if (valSDP <= minval-epslon){ // violation
		sdpEdgeConst.newadd ++;
		sdpEdgeConst.size ++;
...
void SDP_violations_in_LPSOlution(T_Instance &instance, int &NbIneqMax,Pop_Ineq_SDP &PopSDP, const int & type)
{
  double rh_cst,
...
//	  cout << "eigVal = " << eigVal; cin.get();
      if (eigVal >=epslon ){ // eigenvalue is negatif, so there is a violation of the Semidefiniteness
		counterNeg++;
...
void FindNegatifEigenvaleu_Chordal(T_Instance &instance,Pop_Ineq_SDP &PopVecProp)
{
...
void Eigenvaleu_ForSomeVertices(T_Instance &instance,Pop_Ineq_SDP &PopVecProp,const  std::vector<int> &Vec_Vertices, const double & frac)
{
...
//void Eigenvaleu_ForSomeVertices(T_Instance &instance,Pop_Ineq_SDP &PopVecProp, std::vector<int> Vec_Vertices, const double & frac)
//{
...
void Gershgorin_Circle(T_Instance &instance,Pop_Ineq_SDP &PopVecProp)
{
  ...
  if (nbOfVertices > instance.DIM) nbOfVertices = instance.DIM;
  ...
  vector<int> aux_vec;
  set<RankVertex> ranked;
  std::set<RankVertex>::iterator it2;
  double sum;
  ...
  for (int i=0; i < instance.DIM; i++){
    sum = 0.0;
    for (int j=0; j < instance.DIM; j++)
      if (i!= j) sum += ValueXij(i+1, j+1, instance);

    RankVertex rkVer(i, sum);
    ranked.insert(rkVer);
  }
  ...
  for (int i=0; i < nbOfVertices  && it2!=ranked.end(); ++it2){
    ...
    //Random
    random = (rand()%10);
    if (random <6) flag = 1; // we give prob of 70%

    if (ite ==0 ) flag =1; //In first iteration we take all the best

    if (flag ==1){ //we do not take a element if it give us a triangle violation
...
void FindNegatifEigenvaleu_antigoEing(T_Instance &instance,Pop_Ineq_SDP &PopVecProp)
{
  //calcule eigenvalues and eigenvector is made by EIGEN (http://eigen.tuxfamily.org)
   ...
   //cout << "The eigenvalues of A are:" << endl << es.eigenvalues() << endl;
    ...
    for (int i=0; i<instance.DIM; i++){
     eigVal = real(es.eigenvalues()[i]);
      ...
      if ((eigVal + /*epslon*100*/ 0.001 < 0 ) || (SDP_SEP == 2)){ // eigenvalue is negatif, so there is a violation of the Semidefiniteness
...
void FindNegatifEigenvaleu(T_Instance &instance,Pop_Ineq_SDP &PopVecProp)
{
  //cout << "Begin Eig find"; cin.get();
  //calcule eigenvalues and eigenvector is made by EIGEN (http://eigen.tuxfamily.org)
  ...
  for (int i=0; i<instance.DIM; i++)
    ...
    if (!COMPLET_EIG) {
      for (int i=0; i<instance.DIM ; i++){
	eigVal = es.eigenvalues()[i];
	if (eigVal + 0.001< 0.0 ) 	counterMaxNeg++ ;
	else  		       		break;
      }//end FOR i
...
void FindGapIneqNegatifEigenvalue(T_Instance &instance,Pop_Ineq_SDP &PopVecProp)
{
  //cout << "Begin Eig find"; cin.get();
  //calcule eigenvalues and eigenvector is made by EIGEN (http://eigen.tuxfamily.org)
...
/*
//New way of calculate the eigenvalue for symmetric matrix
void FindNegatifEigenvaleu_RankedVertices(T_Instance &instance,Pop_Ineq_SDP &PopVecProp)
{
...
//New way of calculate the eigenvalue for symmetric matrix
void FindNegatifEigenvaleu_Constraint_WithPositiveEig(T_Instance &instance,Pop_Ineq_SDP &PopVecProp)
{
  //cout << "Begin Eig find"; cin.get();
  //calcule eigenvalues and eigenvector is made by EIGEN (http://eigen.tuxfamily.org)
  ...
  for (int i=0; i<instance.DIM; i++)
	...
	if (eigVal >= 0 )	PopVecProp.SetIneq(es.eigenvectors().col(i), instance.DIM, -eigVal,false ); // add new positive eigenvector in population
	else			PopVecProp.SetIneq(es.eigenvectors().col(i), instance.DIM,  eigVal, true); // add new eigenvector in population
...
void Design_SDPIneq_SubGraph ( T_Instance &instance,Eigen::VectorXcd &vecProp,  const int begin, const int endd)
{
 ...
 // vec_vertices {u | v_1, v_2, ... , V_n} where u is the hub of the wheel
  ...
  for (int i=0; i<size_T; i++)
	...
	if (valvp != 0.0)
	{
	  Const.vars[counter] = (MSKidxt) aux_edge;
...
void FindNegatifEigenvaleuSubGraph(T_Instance &instance,Pop_Ineq_SDP &PopVecProp)
{
  //calcule eigenvalues and eigenvector is made by EIGEN (http://eigen.tuxfamily.org)
  MinVP = 1;
  vector<double> egVec(instance.DIM,0.0);
  ...
  for(int j=0; j+step-1<instance.DIM;j+=step){ // it start by 1
   ...
   //cin.get();
    ...
    for (int zz=0; zz<step; zz++){
      ...
      if ((eigVal + /*epslon*100*/ 0.001 < 0 )){ // eigenvalue is negatif, so there is a violation of the Semidefiniteness
...
void FindNegatifEigenvaleuSubGraph_triangle(T_Instance &instance,Pop_Ineq_SDP &PopVecProp)
{
  //calcule eigenvalues and eigenvector is made by EIGEN (http://eigen.tuxfamily.org)
  MinVP = 1;
  MatrixXd mat_X(instance.DIM,instance.DIM);
  ...
  //Tranformation x to X
    ...
    for(int j=0; j<instance.DIM-2; j++){ // it start by 1
      for(int i=j+1; i<instance.DIM-1; i++){//j+1 pois nao qro xii
	  for(int h=i+1; h<instance.DIM; h++){//i+1 pois nao qro xhh
...
inline void SetPretreatmen(T_Instance & instance)
{
  ...
  if ((PreVP != 0) &&(PreVP < 15)){     //Pre-treatment
...
//Euclidian norm
   ...
   //** Making the i-1 new vectors from a comination of eigenvectors of matrix
    for (int j=0; j< instance.DIM; j++){ //elements of new vector
      vecXc = mat_X.col(j);
      for (int i=0; i<instance.DIM; i++)
	egVec[i] = vecXc[i];//
...
void set_pre_inequalities_VP3(T_Instance &instance)
{
 ...
 //** Making the i-1 new vectors from a comination of eigenvectors of matrix
  for (int i=0; i < instance.DIM; i++){ // we must start from the second element in the Matrix
    for (int j=i+1; j< instance.DIM; j++){ //elements of new vector
	...
	if (val > 0){
	  ...
	  if ( (egVec[i] ==0) && ( egVec[j] ==0)) egVec[i] = 1.0/SqNorm;
	  else if (egVec[i] == 0) egVec[i] = egVec[j];
...
void Method_to_ValidEigenvalues_Vilmar(T_Instance &instance,Pop_Ineq_SDP &PopVecProp)
{
  double val;
  double SqNorm = sqrt(double (instance.DIM -1));// it has n-1 elements different of zero
...
//** Combination of the eigenvectors that are negatives
void Method_to_ValidEigenvalues_Miguel(T_Instance &instance,Pop_Ineq_SDP &PopVecProp)
{
 ...
 //** Making the i-1 new vectors from a comination of eigenvectors of matrix
  for (int i=0; i < (int)MatrixVec.size(); i++){ // we must start from the second element in the Matrix
    abs = 0.0;
...
void Heuristic_to_ValidEigenvalues(T_Instance &instance,Pop_Ineq_SDP &PopVecProp)
{
  //calcule eigenvalues and eigenvector is made by EIGEN (http://eigen.tuxfamily.org)
  int random1, random2;
...
void Design_SDPIneq33( T_Instance &instance,const std::vector<double> &vecProp, double& val)
{
 ...
 // vec_vertices {u | v_1, v_2, ... , V_n} where u is the hub of the wheel
  ...
  counter = 0;
  // making sum(u,v_i)
  for (int i=0; i<instance.DIM; i++) if ((vecProp[i] <= -epslonEig) || (vecProp[i] >= epslonEig))
    for (int j= i; j< instance.DIM && vecProp[i] != 0.0; j++){
	...
	if (aux_edge != -1){ // (-1) means that edge does not exist in graph
...
inline void print_Inequality_Tconstraint(const T_constraint &Const)
{
  ...
  if (Const.bkc == MSK_BK_LO) cout << " >= " << Const.blc ;
  else if(Const.bkc == MSK_BK_FX) cout << " = " << Const.blc ;
       else cout << " <= " << Const.buc ;
...
inline void select_vertices_fromMOstviolatedIneq(const T_Instance &instance, std::vector<int> &ranked_v, const int & max_rand, const int & qt_Ineq, const int & max_size,
						  Pop_Data_ineq &PopBestIneq_origem, Class_PopTriangle 	&PopTriangle_origem, Class_PopClique &PopClique_origem ,Class_PopClique &PopGenClique_origem,
						  Pop_Ineq_Wheel &PopWheell_origem, Pop_Ineq_Wheel &PopBiWheell_origem)
{
  int max = max_rand;
  int cont, Seq;
...
{
  if (i==0){ // lecture
     if (SDP_SEP == -1) cout << "SDP solver," <<endl ;
     if (SDP_SEP == -2) cout << "LP_IPM_modification solver," <<endl ;
     if (SDP_SEP == 0) cout << "LP_original solver," <<endl ;
     if (SDP_SEP >= 1) cout << "LP_Eigenvalue solver," <<endl ;
...
void WriteFinalResult_File (const std::string &fileResult,const T_Instance &instance, clock_t &time, const int ite)
{
  ...
  myfile << endl << endl << "   Total_time_of_execution = " << (double) (clock()-time)/CLOCKS_PER_SEC << " sec"  ;
  myfile << endl << "Time_Sep_wheel " << Time_WHE << " sec" ;
  myfile << endl << "Time_Sep_Biwheel " << Time_BWH << " sec" ;
  myfile << endl << "Time_Sep_Clique " << Time_CLI << " sec" ;
  myfile << endl << "Time_Sep_GenClique " << Time_GCL << " sec";
  myfile << endl << "Time_Sep_Triangle " << Time_TRI << " sec";
  myfile << endl << "Time_SEP_EIG " << Time_SDPCUT << " sec";
  myfile << endl << "Time_IPM " << Time_IPM << " sec";
  myfile << endl << "MAXTIME " << MAXTIME << " sec";
...
void Bi_Wheel_InequalitySelective(T_Instance &instance,int &NbIneqMax, const int &type, Pop_Ineq_Wheel &PopWh)
{
...
void Wheel_InequalitySelective_Heuristics(T_Instance &instance,int &NbIneqMax,  const int &type, Pop_Ineq_Wheel &PopWh)
{
...
bool Wheel_Heuristic_MultipleSize (const T_Instance &instance, std::vector<int> &vec_vertices,const int &typeWheel, Pop_Ineq_Wheel & PopWh )
{
  bool 		end = false;
  ...
  while (!end)
  {
	...
	if (std::size_t (pos) == vec_vertices.size()-1){
	  ...
	  rh_cst = (double) typeWheel*((int) (aux_vec.size()-typeWheel)/2);// It is the righ hand of ineq ( wheel <= rh_cst = (typewheel* Lowe(q))); q is the size of wheel
	    if (sum2> rh_cst+epslon){// Wheel inequality violated
...
void  GENETIC_Wheel(const T_Instance &instance, std::vector<int> &vec_vertices,const int &typeWheel, const int &SizeCycleInWheel, Pop_Ineq_Wheel & PopWh)
{
  int SizePop = 20;
  double	sum,rh_cst;
  Pop_Ineq_Wheel PopGen;
  vector<int>	aux_vec;
...
void Genetic_LocalSearch_Wheel(const T_Instance &instance,const int &typeWheel, const int &SizeCycleInWheel,
			       Pop_Ineq_Wheel & PopWh,const double &rh_cst, const int & SizePop )
...
inline void SelectMama_and_Papa(std::vector<int> &VecPapa, std::vector<int>&VecMama, Pop_Ineq_Wheel &PopWh,
...
void  GRASP_Wheel(const T_Instance &instance, std::vector<int> &vec_vertices,const int &typeWheel, const int &SizeCycleInWheel,
		  Pop_Ineq_Wheel & PopWh,const double &rh_cst, const int & NbItGrasp )
{
  // GRASP =
  ...
  for (int i=0; i<NbItGrasp;i++){
    ...
    if (Construction_GRASP_Wheel (instance, vec_vertices, typeWheel, SizeCycleInWheel)){ // the construction was well suceeded !!!
...
inline double Evaluation_Wheel (const T_Instance &instance, std::vector<int> &vec_vertices,const int &typeWheel)
{
  ...
  for (int j=typeWheel; std::size_t (j)<vec_vertices.size(); j++)
    ...
    }else{ sum -= ValueXij(vec_vertices[j],vec_vertices[j-1],instance);}
...
bool Construction_GRASP_Wheel(const T_Instance &instance, std::vector<int> &vec_vertices,const int &typeWheel, const int &SizeCycleInWheel)
{
  ...
  Class_Set_RCL 	rcl; // List RCL to add a random element
  double	sum;
  int		AuxJ;
  ...
  for (int i=1; i< (int)vec_vertices.size();i++) // it should start in the typewheel element of the vec_vertices (change to make it faster to bicycle wheel)
  {
	...
	rcl.addElement(h, sum);
     ...
     if (AuxJ == -1){ // there is no vertex to be added in the wheel
       return false;
     }else vec_vertices[i] = AuxJ; // add in the vec_vertices
				 ...
				 const int &h, const int & PosInVec)
{
  ...
  if (!ISCOMPLETE_GRAPH){//just check if graph is not complete
    { // Verifing if there exist edge with the central vertex
...
void AddAll_TriangleInequality( T_Instance &instance,const int &NbIneqMax, const int &type, Class_PopTriangle &PopTri)
{
  ...
  set<IneqTriangle>::iterator 	it;

    //Verifying all the combinations, i.e, all the triangles in edge
    for(int j=1; j<=instance.DIM-2; j++){ // it start by 1
      aux_vec_vert.clear();
	...
	if (!get_exceptionsVector_vertex(instance, aux_vec_vert, i)){
	  ...
	  for(int h=i+1; h<=instance.DIM; h++){//i+1 pois nao qro xhh
	    if (!get_exceptionsVector_vertex(instance, aux_vec_vert, h)){ //verifing if the edges exist
...
void TriangleInequalitySelective( T_Instance &instance, int &NbIneqMax, const int &type, Class_PopTriangle &PopTri)
{
  // TRIANGLE CONTRAINT -> X_ij + X_hj - X_hi <= 1
  ...
  set<IneqTriangle>::iterator 	it;
    ...
    //Verifying all the combinations, i.e, all the triangles in edge
    for(int j=1; j<=instance.DIM-2 && PopTri.size() <= NBMAXINEQ; j++){ // it start by 1
      aux_vec_vert.clear();
      aux_vec_vert.push_back(j);
      for(int i=j+1; i<=instance.DIM-1 && PopTri.size() <= NBMAXINEQ; i++){//j+1 pois nao qro xii
	if (!get_exceptionsVector_vertex(instance, aux_vec_vert, i)){
	  aux_vec_vert.push_back(i);
	  for(int h=i+1; h<=instance.DIM; h++){//i+1 pois nao qro xhh
	    if (!get_exceptionsVector_vertex(instance, aux_vec_vert, h)){ //verifing if the edges exist
...
void DesignTriangleIneq_Mosek ( T_Instance &instance,const IneqTriangle &IneqT, const double &cstT)
{
 // Disign in CPlex the inequality:
...
inline bool  get_exceptionsVector_vertex(const T_Instance &instance, std::vector<int> &vec_vertices, const int &h)
{
  ...
  for (std::size_t i=0; i<vec_vertices.size(); i++){
    ...
    if (instance.indice_cij[aux_edge] == -1){ // if true it means that it does not exist
      return true;
...
void GenClique_Inequality( T_Instance &instance, int &NbIneqMax, const int &type, Class_PopClique &PopGen)
{
...
void Clique_InequalitySelective_Heuristic (T_Instance &instance, int &NbIneqMax, const int &SizeClique, const double &cst, const int &type, Class_PopClique &Population )
{
  int		count=0;
  double	sum;
...
void Design_Representative_OnePerCluster(T_Instance &instance)
{
  ...
  for (unsigned j=0; j< instance.DIM; j++){
    ...
    Const.vars[counter] = (MSKidxt) instance.edge_nb + j; // it is the position of r_j in our variables vector
    Const.aval[counter] = +1.0;

    counter ++;
    //setting the sum of inequality
    for (unsigned i=0; i<j; i++){
	I = i+1; J = j+1; // start with 1
	aux_edge = instance.indice_cij[I+J*(instance.DIM+1)];
	Const.vars[counter] = (MSKidxt) aux_edge;
	Const.aval[counter] = +1.0;
	counter++;
...
void Design_RepresentativeIneq_MaxOnePerCluster(T_Instance &instance)
{
  ...
  for (unsigned j=0; j< instance.DIM; j++)
    ...
    //Inserting the representative variable (rj)
    Const.vars[0] = (MSKidxt) instance.edge_nb + j; // it is the position of r_j in our variables vector
    Const.aval[0] = +1.0;
...
void Design_RepresentativeIneq_SumRepresentative(T_Instance &instance, const int & startRepresVars)
{
  ...
  //creating a constraint
    ...
    //setting the sum of inequality
  for (unsigned j=0; j< instance.DIM; j++){
    Const.vars[j] = (MSKidxt) startRepresVars + j;
    Const.aval[j] = +1.0;
  }
...
void Design_ExtendLimitEdge(T_Instance &instance)
{
  ...
  for (unsigned j=0; j< instance.DIM; j++)
    for (unsigned i =0; i < j; i++) if (ExistEdge(instance, i, j) && i != j){
...
void Design_ExtendLimitRepresentative(T_Instance &instance)
{
  ...
  for (unsigned j=0; j< instance.DIM; j++)
    ...
    Const.vars[0] = (MSKidxt) aux_edge; // it is the position of r_j in our variables vector
    Const.aval[0] = -1.0;
...
void Design_ExtendEqualIneq(T_Instance &instance)
{
  ...
  for (unsigned j=0; j< instance.DIM; j++){
	...
	I = i+1; J = j+1; // start with 1
	aux_edge = instance.indice_cij[StartExtendVars + I+J*(instance.DIM+1)];
	Const.vars.push_back((MSKidxt) aux_edge);
...
void DesignCliqueIneq_MOSEK (T_Instance &instance,const std::vector<int>  &vec_vertices, const double &cstCliq)
{
 ...
 //	sum{vec_vertices} >= 1.0; // for clique inequalty
  ...
  //cout << "entrou na inequacao " << endl;
  int 		I, J, aux_edge;
  if (cstCliq == 1.0) 	TOTAL_CLI++; //clique
  else		   	TOTAL_GCL++; // generalclique
  ...
  for (std::size_t i=0; i<vec_vertices.size(); i++)
    for (std::size_t j=i+1; j<vec_vertices.size(); j++){
      I = vec_vertices[i]; J = vec_vertices[j];
      aux_edge = instance.indice_cij[I+J*(instance.DIM+1)];
      Const.vars[counter] = (MSKidxt) aux_edge;
      Const.aval[counter] = +1.0;
      counter++;
			...
			const int &Size_Clique, const std::set<int> &fbdn_edges  )
{
  ...
  //testting each vertex
   for (int h=1; h<=instance.DIM;h++){ //remember that in data file vertices start by 1 not 0
	 ...
	 if (sum < best){//Testing the improvement
	   vec_vertices = aux_vec;
	   i = vec_vertices.size();
				   ...
				   const int &Size_Clique, const std::set<int> &fbdn_edges )
{
  ...
  while ((vec_vertices.size()< std::size_t (Size_Clique))&&(!end)) {
    ...
    }else { // there is no edge with satisfying conditions for last edge
...
inline void setSolver(T_Instance &instance, MSKrescodee  &r, MSKtask_t    &task, MSKenv_t &env, const int & Option)
{
  ...
  switch(flag)
  {
    ...
    case 3:
 	  //cout << "Start Cplex";
	  solveCPLEX_LP(instance); //Simplex method
	  break;
    case -4:
	  SolveMosek(instance, r,task, env);
	  break;
...
//    SIMPLEXITE = 1; // change to never do that again in simplex mode
  ...
  if ( r==MSK_RES_OK ){
    ...
    if (/*NEW_TASK_LP*/true){ //the idea of reusing the same task is not helpfull -
	// Create the optimization task.
...
//       r = MSK_linkfunctotaskstream(task,MSK_STREAM_LOG,NULL,printstr);
 ...
 //Changing number of MSK_IPAR_NUM_THREADS (it should be done after the optimization)
    ...
    if (solveBySIMPLEX){ // Solve by Simplex
      r = MSK_putintparam(task,MSK_IPAR_OPTIMIZER,MSK_OPTIMIZER_FREE_SIMPLEX);
 ...
 //cout << "before R = " << r<< endl;
	...
	if (solsta  != MSK_SOL_STA_OPTIMAL){
	  //r = MSK_getsolsta (task,MSK_SOL_ITR,&solsta);
	    //cout << "solsta = " << solsta << "MSK_SOL_STA_NEAR_OPTIMAL = " << MSK_SOL_STA_NEAR_OPTIMAL << endl; cin.get();
...
inline void SolveMosek2(T_Instance &instance, MSKrescodee  &r, MSKtask_t    &task, MSKenv_t &env)
{
  ...
  if ( r==MSK_RES_OK ){
    //NEW_TASK is a global variable
    if (/*NEW_TASK_LP*/ true ){ // the idea of NEW_TASK_LP is not good (here, it should be always true)
      // Create the optimization task.
      r = MSK_maketask(env,(MSKint32t) instance.CONST.size(),(MSKint32t) instance.totalVars,&task);
...
double  Hybrid_GraspAndTabu(const T_Instance &instance)
{
  ...
  for (unsigned j=0; j < NbIte_GRASP; j++){
    Partitions.clear();
    if (j %2 == 0) Initial_solution_to_GraspFeasible (instance,0, 0.5, Partitions);
    else           GenerateRandomSolution (instance, Partitions);
    LocalSearch_Solution (instance, Partitions);
...
bool Tabu_ModifyOneVertex_best(const T_Instance &instance, std::vector< std::vector<int> > &Partitions, std::vector<int> &TabuList, 	const int &TabuListSize )
{
  ...
  for (unsigned i=0; i < Partitions.size(); i++ ){
     if ( Partitions[i].size() <= 1) continue; //Will change just if Partition size is bigger than 1

     for (unsigned j=0; j < Partitions.size(); j++ ){
       if (i == j) continue; //do nothing

       for ( unsigned ii =0; ii< Partitions[i].size(); ii++){
         if ( TabuList[ Partitions[i][ii] ] != 0) continue; //not autorized move
	  sumEdge = 0.0;
         for ( unsigned jj =0; jj< Partitions[j].size(); jj++){
...
double Grasp_Heuristic_FeasibleSoltion (const T_Instance &instance)
{
  ...
  for (unsigned i=0; i < NbIte_Grasp &&  getCurrentTime_Double(start2) <= maxTimeGRASP; i++ ){
    ...
    if (i %2 == 0) Initial_solution_to_GraspFeasible (instance,0, 0.5, Partitions);
    else           GenerateRandomSolution (instance, Partitions);
    LocalSearch_Solution (instance, Partitions);
...
double gain_movePartition(const T_Instance &instance,  const std::vector< std::vector<int> > &Partitions, const int &vert, const int &Par_or, const int &Par_target)
{
	...
	I = vert+1;
      for ( unsigned iii =0; iii< Partitions[Par_or].size(); iii++){
		if (Partitions[Par_or][iii] == vert) continue;
...
//       cin.get();
     ...
     while ((!select) &&(Counter < max_sizeRCL) &&(Counter <RCL_List.size()) ){
      ...
      if (random <= RCL_prop){//selected
		select = true;
		it=RCL_List.begin();

		for (int zz=0; zz<Counter; zz++ ) ++it;
...
//   cout << "instance.DIM = " << instance.DIM; cin.get();
  ...
  for (it=setICHvar.begin(); it != setICHvar.end(); ++it){
    if ((*it).sumEdge < epslonICH) break; // exit for iteration
...
//       cout <<"cegou33"; cin.get();
	...
	if (Partitions.size() < instance.DIM){ //it is the final feasible solution
		LocalSearch_Solution (instance, Partitions);
              Change_VecVertexOrigem_by_Partition_ICH (Partitions, OriginVertices);
	...
	}else { // it is Partitions.size() == instance.DIM and we need to increase the epslonICH
		if (epslonICH*0.75 < 0.5){ // Error the method is not accurate at all !!
...
//   cout << "instance.DIM = " << instance.DIM; cin.get();
	...
	while (CONTINUE){
		...
		bestVertex  = -1;
		for (unsigned j= i+1; j <instance.DIM; j++){
	  		if (setVertex[j] == true) continue; // if already set we are not going to verify it
	  		counter = 0;
	  		sumVal = 0.0;
...
inline void PrintPartitionICH(const std::vector< std::vector<int> > &Partitions)
{
  ...
  cout << "Partitions.size()" <<Partitions.size() << endl << "Printing partions .." << endl;
...
//    SIMPLEXITE = 1; // change to never do that again in simplex mode
      ...
      // Append 'numcon' empty constraints.
    //The constraints will initially have no bounds.
  if (start == 0){ // not a simplex type
       if ( r == MSK_RES_OK )	       r = MSK_appendcons(task,(MSKint32t) instance.CONST.size());
  }else {
      /* Append a new constraint */
      if (r == MSK_RES_OK)
...
inline void setObjFunction_andVarBounds__LPmosek(const T_Instance &instance, MSKrescodee  &r, MSKtask_t    &task, MSKenv_t &env, int Form_Type)
{
  ...
  // add a constant term to the objective.
   ...
   //Check the type
  if (Form_Type == 0){ //Edge only formulation
	...
	r = MSK_putcj(task,j,+1.0*instance.cij.barc_v[j]);
      ...
      // Set the bounds on variable j.
      //  blx[j] <= x_j <= bux[j]
      if(r == MSK_RES_OK){//edge only formulation
	if (j < instance.DIM*K){ // Xip variables
	  r = MSK_putvarbound(task,
			      (MSKint32t)j,           // Index of variable.
			      MSK_BK_RA,      // Bound key.
			      0.0,      // Numerical value of lower bound.
...
void Lire_Instance(const std::string nomFichier,T_Instance& instance)
{
  ...
  for (int i=0;i<instance.edge_nb; i++)
  {
    ...
    //indice
    if (fst != scon && (weight  < -epslon || weight > epslon) ){ // do not accept loop or weight==0
	...
	//index of edge to find it easely
      ...
      }else {//@TODO:Execption if edge is repeated
...
inline void noCompleteGraph(T_Instance& instance)
{
  ...
  for (int i=1;i<instance.DIM+1; i++)
    for (int j=i+1;j<instance.DIM+1; j++){
      ...
      if (instance.cij.barc_v[index] == 0.0 ){//there is no edge (thus we need to add one with cost of 0)
...
//
    ...
    cout << "End of set_newFormulation "; cin.get();
...
// 	cout << ", j = "<<(*instance).cij.barc_i[aux_edge]  << ", p = "<<(*instance).cij.barc_j[aux_edge] << ", val_aij = " << val_aij ; // cin.get();
    ...
    cout << ", p= "<<(*instance).cij.barc_i[aux_edge]  << ", p = "<<(*instance).cij.barc_j[aux_edge] <<  "aux_edge = " << aux_edge <<endl ; cin.get();
...
inline double calcule_IncidenteEdges(const T_Instance &instance, std::vector<double> *posM, std::vector<double> *negM, const std::vector<int>  &ind_cij, const T_cost_Matrix &costMatrix)
{
  ...
  for (unsigned i=0; i<instance.DIM-1; i++){
    I = i+1;
    (*posM)[i] == 0; (*negM)[i] == 0;
    for (unsigned j =i+1; j<instance.DIM; j++) if (i!= j){
      J = j+1;
      aux_edge_cij = ind_cij[I + (J)*(instance.DIM+1)];
...
void Design_Impose_EdgeCut_if_DifferentPartition(T_Instance &instance)//Ineq 2.8 and 2.9 together in thesis
{
  ...
  for (unsigned j=0; j< instance.DIM; j++)
    for (unsigned i =0; i< instance.DIM; i++) if(ExistEdge(instance, i, j) && i != j){
      for (unsigned p=0; p<K; p++){
	...
	//First node variable
	P = p+1; V = i+1; // start with 1
	aux_edge = instance.indice_cij[StartNodeVars + V + P*(instance.DIM+1)];
	Const.vars[1] = (MSKidxt) aux_edge;
	if (i<j)Const.aval[1] = +1.0;
	else 	Const.aval[1] = -1.0;
	counter++;


	//second node varirable
	P = p+1; V = j+1; // start with 1
	aux_edge = instance.indice_cij[StartNodeVars + V + P*(instance.DIM+1)];
	Const.vars[2] = (MSKidxt) aux_edge;
	if (i<j) Const.aval[2] = -1.0;
	else 	 Const.aval[2] = +1.0;
	counter++;
...
void Design_Impose_EdgeEqualOne(T_Instance &instance)//Ineq 2.7 in thesis
{
  ...
  for (unsigned j=0; j< instance.DIM; ++j)
    for (unsigned i =j+1; i< instance.DIM; ++i) if(ExistEdge(instance, i, j) && i != j){
      for (unsigned p=0; p<K; p++){
...
//   printMatrix(instance.indice_cij, instance.DIM+1); cin.get();
   ...
   //remeber that edges start with 1 not 0 !!!
    for (int i=1;i<instance.DIM+1; i++)
    for (int j=i+1;j<instance.DIM+1; j++) if(ExistEdge(instance, i-1, j-1)){
...
if (instance.DIM == 0){
	...
	cout << "Trivial solution is = " << instance.sum_cost << endl << " Thks ! (Press any thing to finish)"; cin.get();
  ...
  for (int i=0;i<instance.DIM ; i++)
    if  (aux_vertex[i] == 0){ // edges in
     ...
     for (int z =0; z < vert_in_block.size(); z++){ //for each vertex of the block we will see if they have any
	 //printvector(vert_in_block); cin.get();
        for (int j=0;j<instance.DIM; j++){

           I = vert_in_block[z]; J = j+1;
    ...
    if((vert_in_block.size() < maxSizeBlock)  || (i == instance.DIM -1) ){ // it catches one block (it is good, so we shoould create a new instance)
...
// 	cout << "It was possible to divide the graph in  (" <<  vec_instances.size() << ") subgraphs ";
  ...
  for (int i=0; i<vec_instances.size(); i++){
	if (i == vec_instances.size() -1) 	PRINT_ITERATIONS = true; //just the last block will appear into the optimization
	else					PRINT_ITERATIONS = false;

	if (vec_instances.size() > 1) K_coreElimination(vec_instances[i]); //try again to eliminate more vertices
...
inline void K_coreElimination(T_Instance& instance)
{
  ...
  while (repeat){
	...
	I = i+1; J = j+1;
	aux_edge = instance.indice_cij[I + (J)*(instance.DIM+1)];
	if (aux_edge!= -1 && (val = GetEdgeWeight(instance, I, J)) != 0){//there existe an edge
	  counter++;
	  if (val > 0) // just positive
	    sumVertex += val;
	}
...
{
int aux_edge ;
	 cout << endl << "printCostMatrix  with Dim = " << instance.DIM  <<", and edge_nb= " << instance.edge_nb << endl;
  for (int i=0;i<instance.DIM ; i++)
    for (int j=i+1;j<instance.DIM ; j++){
...
{
  cout << endl << "printCostMatrix  with Dim = " << instance.DIM << endl;
  for (int i=0;i<instance.DIM ; i++)
    for (int j=i+1;j<instance.DIM ; j++)
      cout << "( " << i+1 << "," << j+1 << ") =" << GetEdgeWeight(instance, i+1, j+1) << endl;
...
inline void Chordal_extention (T_Instance& instance)
{
	...
	for (int i=0; i < instance.DIM -2; ++i){ // (-2) because it can creat at most
...
inline void Add_edgeInstance (T_Instance& instance, const int & J, const int & I)
{
	...
	if (instance.indice_cij[i + (j)*(instance.DIM+1)] == -1){//there is no edge (thus we need to add one with cost of 0)
