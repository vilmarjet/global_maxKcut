...
public:
    ...
    void set_edge_variables()
    {
        ...
        {
            const GraphEdge *edge = edges->get_edge_by_index(i);
            this->solver->add_variable(edge, Variable(i,
                                                      0.0,
                                                      1.0,
                                                      edge->get_weight(),
    ...
    void find_violated_constraints(const int &nb_max_ineq)
    {
        ...
        for (std::size_t idx_ineq = 0; idx_ineq < inequalities_type.size(); ++idx_ineq)
        {
            ...
            inequalities_type[idx_ineq]->find_violated_constraints(this->solver,
                                                                   this->instance,
        ...
        for (std::set<ViolatedConstraint *, CompViolatedConstraint>::iterator it = violated_constraints.begin();
             ...
             ++it, ++ counter_ineq)
        {
            // std::cout << (*it)->get_constraint()->to_string(this->instance->get_graph()) << "\n";
